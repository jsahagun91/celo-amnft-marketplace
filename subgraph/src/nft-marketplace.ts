...
export function handleListingCreated(event: ListingCreated): void {
  ...
  // The NFT Contract Address + Token ID + Seller Address can be used to uniquely refer to a specific listing
  const id = 
...
export function handleListingCanceled(event: ListingCanceled): void {
  ...
  // from when the listing wa first created
  const id = 
    event.params.nftAddress.toHex() +
    "-" +
    event.params.tokenId.toString() +
...
export function handleListingPurchased(event: ListingPurchased): void {
  // Recreate the ID that refers to the listing
  // Since the listing is being update, the datastore must alreadt have an entity with this ID
  // from where the listing was first created
  const id = 
    event.params.nftAddress.toHex() +
    "-" +
    event.params.tokenId.toString() +
